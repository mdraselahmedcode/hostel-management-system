...
function sendStudentCheckInNotification($studentId, $hostelId, $floorId, $roomId) {
    global $conn;
   ...
   // 4. Get current roommates (excluding this student)
    ...
    $body = "
        Hi {$firstName},<br><br>
        Welcome to your new hostel accommodation. Your check-in has been <strong>successfully completed</strong>.<br><br>
    ...
    if (!empty($roommates)) {
        ...
        foreach ($roommates as $mate) {
            $body .= "<li>{$mate['first_name']} ({$mate['email']}, Phone: {$mate['contact_number']})</li>";
        }
        $body .= "</ul><br>";
    }
    ...
    $body .= "
        <br><br>You are now officially checked into the hostel.<br>
        You can log in to the student portal using your credentials.<br><br>
        <a href='" . BASE_URL . "/student/login.php'>Click here to log in</a><br><br>
        Regards,<br>Hostel Admin Team
    ";

    // 6. Send the email
    return sendEmail($email, "Hostel Check-In Confirmed", $body);
}
